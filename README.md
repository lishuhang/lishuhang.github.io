...
### v6.5.0 (2026-04-26)
- **首页加载速度优化**: 将 1MB 内嵌 `__POSTS__`/`__TI__`/`__YI__` 数据移至独立 `/assets/data/posts.js` 文件异步加载，首页 HTML 体积从 1.2MB（888KB gzip）降至约 200KB（28KB gzip），首屏渲染时间大幅缩短
- 首页预渲染的 10 张卡片立即可见，翻页器在数据异步加载后出现
- 筛选页（`?year=`/`?tag=`/`?search=`）显示加载提示，数据到达后渲染筛选结果
- 数据加载失败时显示"刷新重试"提示
- 浏览器可独立缓存 `posts.js`，后续访问零延迟
- 文章页不加载 `posts.js`，无额外开销
